...
class TestRequestImageSearchUrl(TestCase):
    ...
    def test_successful_request_status_400(self, mock_sys, mock_request):
    ...
    @patch('app.image_downloader.sys.exit')
    def test_successful_request_status_500(self, mock_sys, mock_request):
    ...
    def test_invalid_url_exception(self, mock_request):
        request_image_search_url = Mock()
        request_image_search_url.side_effect = InvalidURL
        with self.assertRaises(InvalidURL):
            request_image_search_url(self.url, self.img)

    def test_timeout_exception(self, mock_request):
        request_image_search_url = Mock()
        request_image_search_url.side_effect = Timeout
...
class BeautifulSoupTest(TestCase):
    ...
    def test_create_beautifulsoup_object(self, mock_request, mock_bs4):
        ...
        self.mock_response = Mock()
        self.mock_response.status_code = 200
        self.response = request_image_search_url(self.url, self.img)
        self.create_beautifulsoup_object = Mock()
        self.soup = create_beautifulsoup_object(self.response)
        self.assertTrue(isinstance(self.soup, bs4.BeautifulSoup))
