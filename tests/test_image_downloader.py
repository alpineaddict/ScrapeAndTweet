...
from tests.samples import ResponseSample, SoupSample
...
class TestUserPrompt(TestCase):
    ...
    def test_user_prompt(self, mock_input):
        ...
        print('\n\nmock called: ', mock_input.assert_called())
...
#         soup = create_beautifulsoup_object(self.response)
...
#         self.image_tag = response.image_tag
#         self.soup = create_beautifulsoup_object(response)

#     def test_image_scrape(self):
#         image_list = scrape_image_urls_and_append_to_list(self.soup)
#         self.assertTrue(isinstance(image_list, list))
#         self.assertIn(self.image_tag, image_list[0])
...
class TestDownloadImages(TestCase):
    ...
    def test_image_download(self, mock_open, mock_request):
        ...
        patch('app.image_downloader.requests.get') as mock_request:
            download_images(self.image_list)

        print(mock_open.assert_called())
        print(mock_request.assert_called())
